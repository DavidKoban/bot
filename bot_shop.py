...
try:
    ...
    def start(message):
        ...
        if message.from_user.id == 5772771813 or message.from_user.id == 266284325:
    ...
    def change_currency(message):
        ...
        def callback_inline(call):
            if call.data == '0':
                ...
                markup1.add(button1, button2)
                bot.send_message(call.from_user.id, "*Выберите криптовалютную сеть:*",
                                 reply_markup=markup1, parse_mode='Markdown')
            elif call.data == '2':
                bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
                markup1 = types.InlineKeyboardMarkup()
                ...
                bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
                msg = bot.send_message(call.from_user.id,
                                       "Напишите колличество BTC в следующем сообщении.\n\nАктуальный курс: " + str(
                                           curses[
                ...
                bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
                msg = bot.send_message(call.from_user.id,
                                       "Напишите колличество ETH в следующем сообщении.\n\nАктуальный курс: " + str(
                                           curses[
                ...
                bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
                msg = bot.send_message(call.from_user.id,
                                       "Напишите колличество USDT в следующем сообщении.\n\nАктуальный курс: " + str(
                                           curses[
            ...
            if message1.text == "Обмен криптовалюты 💱":
                ...
                try:
                    if float(price) >= float(f"{200 / curses[coin1]:.6f}"):
                        ...
                        markup1.add(button1, button2, button3)
                        bot.send_message(message1.from_user.id,
                                         "*Сумма продажи: " + price + " " + coin1.replace('SELL', '') + ".*\n\nЕсли все верно, выберите свой *тип* своих реквизитов в долларах США.",
                                         parse_mode='Markdown', reply_markup=markup1)
                    else:
                        bot.send_message(message1.from_user.id,
                                         'ОШИБКА: неверная сумма депозита. Попробуйте еще раз или свяжитесь @crypdexsupport')
                        callback_inline(bla)
                except ValueError:
                    bot.send_message(message1.from_user.id,
                                     'ОШИБКА: неверная сумма депозита. Попробуйте еще раз или свяжитесь @crypdexsupport')
        ...
        def callback_inline1(call):
            if call.data == '111':
                ...
                bot.clear_step_handler_by_chat_id(chat_id=call.message.chat.id)
                msg = bot.send_message(call.from_user.id,
                                       "Отправьте свой PayPal\n\n🚨Убедитесь, что вы правильно ввели PayPal. В противном случае средства будут потеряны.🚨",
                                       parse_mode='Markdown')
        ...
        def end1(message, coin, price, call_1, user_wallet, a, net):
            bot.send_message(message.from_user.id,
                             "*✅Ваша заявка находится в обработке.* Это может занять около 20 минут.\nСвяжитесь с @crypdexsupport, если вы не получили свои средства.",
                             parse_mode='Markdown')
            time.sleep(2)
            f = json.load(open("manager.txt", "r"))
            for i in f.keys():
                try:
                    bot_manager.send_message(f[i], '@'+str(message.from_user.username) + ' sell ' + price + " " + coin + " send " + a + " USD to "+net+"\n\n" + user_wallet)
                except:
                    pass
        ...
        def haha(message1, price, coin1, network):
            ...
            msg = bot.send_message(message1.from_user.id,
                                   "*Сумма вашей покупки: " + price + " " + coin1.replace('BUY',
                                                                                           '') + ".*\n\nЕсли все верно, отправьте адрес " + coin1.replace(
        ...
        def end_buy(message, call_1):
            ...
            if message.text == "Обмен криптовалюты 💱":
                ...
                if len(user_wallet) >= 30:
                    ...
                    markup1.add(button1, button2, )
                    msg = "Отправьте " + a + " USD на реквизиты:\n\n`" + my_wallet['paypal'] + "` (PayPal)\n\nили\n\n`"+my_wallet['card']+"` (Банковская карта)\n\nПосле подтверждения оплаты будут отправленны" + price + " " + coin + " на реквизиты:\n\n" + user_wallet + " (" + network + ")" + "\n\n*Что бы подтвердить оплату, нажмите на Я оплатил✅.*\n\n‼️Если вы не нажмете на Я оплатил✅ после оплаты, ваши средства могут быть потеряны️‼️"
                    bot.send_message(message.from_user.id, msg, reply_markup=markup1,
                                     parse_mode='Markdown')
                else:
                    bot.send_message(message.from_user.id,
                                     'ОШИБКА: неверный адрес кошелька. Попробуйте еще раз или свяжитесь @crypdexsupport')
        ...
        def end1_buy(message, coin, price, call_1, user_wallet, network):
            ...
            for i in f.keys():
                try:
                    bot_manager.send_message(f[i], '@'+str(message.from_user.username) +' купил '+ price + " " + coin + "\n\n" + user_wallet + ' ('+ network +')')
                except:
                    pass
