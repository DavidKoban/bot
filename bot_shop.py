...
bot.set_webhook()
support = "@binance_exchange_dxb_support"
    ...
    def change_currency(message):
            ...
            if message1.text == "Обмен криптовалюты 💱":
                ...
                try:
                    if float(price) >= float(f"{200 / curses[coin1]:.6f}"):
                        ...
                        bot.send_message(message1.from_user.id,
                                         'ОШИБКА: неверная сумма депозита. Попробуйте еще раз или свяжитесь '+support)
                        callback_inline(bla)
                except ValueError:
                    bot.send_message(message1.from_user.id,
                                     'ОШИБКА: неверная сумма депозита. Попробуйте еще раз или свяжитесь '+support)
        ...
        def place(message, coin, price, side):
            bot.send_message(message.from_user.id,
                             "✅Ваша заявка находится в обработке. В ближайшее врем с вами свяжется наш администратор "+support+" для обсуждения деталей встречи.")
            f = json.load(open("manager.txt", "r"))
        ...
        def end(message, call_1, net):
            ...
            if message.text == "Обмен криптовалюты 💱":
                ...
                if len(user_wallet) > 0:
                    ...
                    bot.send_message(message.from_user.id,
                                     'ОШИБКА: неверный адрес. Попробуйте еще раз или свяжитесь '+support)
        ...
        def end_buy(message, call_1):
            ...
            if message.text == "Обмен криптовалюты 💱":
                ...
                if len(user_wallet) >= 30:
                    ...
                    bot.send_message(message.from_user.id,
                                     'ОШИБКА: неверный адрес кошелька. Попробуйте еще раз или свяжитесь '+support)
